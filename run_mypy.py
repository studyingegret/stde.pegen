...
p.add_argument("--first-only", action="store_const", dest="action", const=Action.FIRST,
               help="Run first run only (excluding grammar_parser.py).")
p.add_argument("--second-only", action="store_const", dest="action", const=Action.SECOND,
               help="Run second run only (just grammar_parser.py, filtering likely false-positives).")
p.add_argument("--third-only", action="store_const", dest="action", const=Action.THIRD,
               help="Run third run only (just grammar_parser.py, not filtering likely false-positives).")
...
# Exact code will change if type check flagging strategy for grammar_parser.py changes
def compile_toml():
...
def main(args):
    if args.action != 0:
        ...
        if args.action & Action.SECOND:
            print_header(f"== Second run: just grammar_parser.py, filtering likely false-positives")
            compile_toml()
            r, w = os.pipe()
        ...
        if args.action & Action.THIRD:
            print_header(f"== Third run: just grammar_parser.py, not filtering likely false-positives")
            run(["mypy", "-m", "stde.pegen.v2.grammar_parser"] + args.args,
                stdout=sys.stdout, stderr=sys.stderr)
        if args.action & Action.SECOND:
            print_header("Use --third-only to run mypy on grammar_parser.py without filtering")
