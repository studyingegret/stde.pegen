...
stde.pegen has some special grammatical elements and rules:
...
but this does not happen in stde.pegen's default preset. The reason is that the Python language needs
a custom tokenizer to handle things like indentation boundaries, some special keywords like ``ASYNC`` and ``AWAIT``
...
In the default preset, tokens are restricted to the ones available in the ``tokenize`` module of the
Python interpreter that is used to generate the parser. This means that
tokenization of any parser generated by ``stde.pegen`` must be a subset of the
tokenization that Python itself uses.

``char_based`` preset
*********************

The ``char_based`` preset provides a character-based matching logic, with no inherent tokenization
outside of the grammar.

Customizing tokenization with a custom base parser class
********************************************************

Grammars can write their own tokenization logic 
