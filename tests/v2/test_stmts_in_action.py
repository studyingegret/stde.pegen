...
def test_return_stmt_with_various_alt_formatting_1() -> None:
...
def test_return_stmt_with_various_alt_formatting_2() -> None:
    grammar = dedent("""
    start:
        | a1=a a2=a {
            parts = []
            for char in a:
                if char in a1 and char not in a2:
                    parts.append(char)
            return "".join(parts)
        # Intentionally mixing more styles
        # Wow, the next alt contains nothing but `a`!
        } | a {
           if a == "a":
               return "Welcome back, a!"
           parts = []
